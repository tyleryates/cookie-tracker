...
function buildUnifiedMetadata(store: ReadonlyDataStore, warnings: Warning[] = [], scouts: Record<string, Scout>): UnifiedMetadata {
  const healthChecks: HealthChecks = {
    ...
    unknownOrderTypes: warnings.filter((w) => w.type === WARNING_TYPE.UNKNOWN_ORDER_TYPE).length,
    unknownPaymentMethods: warnings.filter((w) => w.type === WARNING_TYPE.UNKNOWN_PAYMENT_METHOD).length,
    unknownTransferTypes: warnings.filter((w) => w.type === WARNING_TYPE.UNKNOWN_TRANSFER_TYPE).length,
    unknownCookieIds: warnings.filter((w) => w.type === WARNING_TYPE.UNKNOWN_COOKIE_ID).length
  };
