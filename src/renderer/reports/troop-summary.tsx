...
export function TroopSummaryReport({ data }: { data: UnifiedDataset }) {
  ...
  const financialStats: Array<{ label: string; value: string; description: string; color: string }> = [
    { label: 'Packages Credited', value: `${packagesCredited}`, description: `${totalPackages} received + ${totalShipped} shipped + ${troopTotals.donations} donations`, color: '#1565C0' },
    {
  ...
  return (
    <div class="report-visual">
      ...
      <StatCards
        stats={[
          { label: 'Troop Package Sales', value: troopSales, description: `${troopTotals.boothDividerT2G} booth + ${troopTotals.virtualBoothT2G} site`, color: '#7B1FA2' },
          { label: 'Girl Package Sales', value: girlDelivery, description: `${dcDelivery} DC + ${inPerson} in person`, color: '#1976D2' },
          { label: 'Direct Ship', value: totalShipped, description: `${troopShipped} troop + ${girlShipped} girl`, color: '#00838F' },
          { label: 'Donations', value: troopTotals.donations, description: `${troopTotals.donations - girlDonations} troop + ${girlDonations} girl`, color: '#E91E63' },
